...
func Download(dataName string, configFilePath string, destDir string, thread int, retryNum int, noResume bool) error {
	// check param
	if dataName == "" && configFilePath == "" {
		fmt.Println("[ERROR] data name not provide")
		return errors.New("no download target")
	}
	if dataName != "" && configFilePath != "" {
		fmt.Println("[WARNING] data name provide, ignore configPath")
		configFilePath = ""
	}
	...
	if configFilePath == "" {
		fmt.Println("[ERROR] data json config error, please input correct data name or file path")
	...
	if err != nil {
		fmt.Println("[ERROR] ", "load file config error:", err.Error(), "\nfile config path:", configFilePath)
		return err
	}
